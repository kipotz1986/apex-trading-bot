...
class BotRunner:
    ...
    async def _step(self, db: Session):
        ...
        for symbol in symbols:
            try:
                ...
                # 4. Get REAL Portfolio State
                try:
                    portfolio = await self._get_portfolio_state(db, exchange, risk_state)
                except Exception as pe:
                    logger.error("portfolio_state_unavailable_skipping", symbol=symbol, error=str(pe))
                    continue
    ...
    async def _get_portfolio_state(self, db: Session, exchange: ExchangeService, risk_state: RiskState) -> PortfolioState:
        """Fetch actual portfolio data from exchange and DB. Raises on failure — never returns fake data."""
        ...
        # 2. Calculate Daily PnL from DB
        today_start = datetime.combine(datetime.now(timezone.utc).date(), time.min).replace(tzinfo=timezone.utc)
        today_orders = db.query(Order).filter(Order.closed_at >= today_start, Order.status == "CLOSED").all()
        daily_pnl = sum(o.pnl_usd or 0.0 for o in today_orders)
        daily_pnl_pct = (daily_pnl / equity * 100) if equity > 0 else 0.0

        # 3. Calculate Weekly PnL from DB
        week_start = datetime.now(timezone.utc) - timedelta(days=7)
        week_orders = db.query(Order).filter(Order.closed_at >= week_start, Order.status == "CLOSED").all()
        ...
        # 5. Calculate available margin (equity minus committed notional from open positions)
