               ...
               <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                  {timelineData.map((event, i) => (
                     <div key={event.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                        <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#050B0A] bg-white/5 text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10">
                           {event.event_type === 'model_training' ? <GitMerge className="w-4 h-4 text-emerald-500" /> : 
                        ...
                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white/[0.02] border border-white/5 hover:border-white/10 transition-colors p-4 rounded-xl">
                           <div className="flex items-center justify-between mb-1">
                              <span className="text-[10px] font-bold text-white/40 tracking-widest uppercase">{fmtDateShortTime(event.timestamp)}</span>
                              {event.metrics && (
                                 <span className="text-[9px] font-black text-emerald-500/80 bg-emerald-500/10 px-2 py-0.5 rounded">
