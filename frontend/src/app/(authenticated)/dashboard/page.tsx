    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4 fill-black" />}
            Run Sync
          </Button>
        </div>
      </div>
                 ...
                 <>
                   ...
                   <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                         {[
                           { label: "Technical", val: insights?.scores.technical || 0 },
                           { label: "Sentiment", val: insights?.scores.sentiment || 0 },
                           { label: "On-Chain", val: insights?.scores.onchain || 0 }
                         ].map(a => (
                           <div key={a.label} className="space-y-1.5">
                              ...
                              <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                 <div className="h-full bg-emerald-500/50 rounded-full transition-all duration-1000" style={{ width: `${a.val}%` }} />
                              </div>
                           </div>
                         ))}
                      </div>
      ...
      <Card className="bg-[#050B0A]/50 border-white/5 backdrop-blur-md overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 px-8 py-6">
          ...
          <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-[10px] font-black tracking-tighter uppercase">
             {positions?.length || 0} ACTIVE TRADES
          </span>
        </CardHeader>
